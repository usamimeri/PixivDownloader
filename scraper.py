...
def parse_image_list(html):
    ...
    if image_urls:
        for url in image_urls:
            try:
                detail_url=urljoin(BASE_URL,url.group('image_url'))
            except:
                print('不合法的url')
            yield scrape_page(detail_url) #获取图片详情页的信息
def parse_image_url(html,nsfw=False):
    '''根据详情页，返回图片url'''
    # pattern=r'<img.*?src="(.*?)".*?class="sc-1qpw8k9-1 jOmqKq".*?>'
    pattern='<link rel="preload" as="image" href="(?P<image>.*?)">' #返回的竟然是JavaScript生成页面
    if nsfw:
...
def run(nsfw=False):
    ...
    image_html_list=parse_image_list(page_html) #返回例如https://www.pixiv.net/artworks/108826801的html
    for html in image_html_list:
        try:
            url=parse_image_url(html,nsfw)
            save_image(url)
        except:
            continue
